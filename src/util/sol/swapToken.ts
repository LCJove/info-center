...
import { currentEnv, initSdk, txVersion } from '../../config/sol';
...
// '9BTDdYjTgZh8qK7myMiaoxMBYe54E9wXS8KUPCjgHuUV'
export async function routeSwap(tokenId: string) {
  const raydium = await initSdk();
  await raydium.fetchChainTime();

  const inputAmount = '100';
  const SOL = NATIVE_MINT; // or WSOLMint
  const [inputMint, outputMint] = [SOL, new PublicKey(tokenId)];
  ...
  if (poolData.ammPools.length === 0) {
    ...
    poolData =
      currentEnv.cluster === 'mainnet'
        ? await raydium.tradeV2.fetchRoutePoolBasicInfo()
        : await raydium.tradeV2.fetchRoutePoolBasicInfo({
